...
export class EmailHtmlGeneratorService {
  ...
  getEmailHtml(emailElements: EmailElements, isPreview = false) {
    const bgColor = emailElements.general.background
      ? emailElements.general.background
      : ConstantsData.bgPickerValue;
    const contentBgColor = emailElements.general.contentBackground
      ? emailElements.general.contentBackground
      : ConstantsData.contentBgValue;
    ...
    <body width="100%" style="margin: 0; padding: 0 !important; mso-line-height-rule: exactly; background-color: ${bgColor};">
        ...
        <![endif]-->
            ...
            border-right: 5px solid ${bgColor};" class="email-container">
                ...
                `;
    if (
      emailElements.general &&
      emailElements.general.logo &&
      emailElements.general.logo.src
    ) {
    ...
    if (
      emailElements &&
      emailElements.structures &&
      emailElements.structures.length
    ) {
  ...
  getLogo(logo: Logo) {
    ...
    <img src="${logo.src}" width="${
      logo.width ? logo.width : ""
                ...
                style="height: auto;max-width:100%;font-family: ${
                  this.defaultFont
                }; font-size: 15px; line-height: 15px;"  alt="${
      logo.altTxt ? logo.altTxt : "image"
    }" onerror="this.src='';">
    `;
    ...
    return `
    <tr>
      <td style="padding: 0 20px 20px 20px; text-align: ${
        logo.align ? logo.align : "center"
      }">
        ${linkText}
      </td>
    </tr>`;
  ...
  getFooter(footer: Footer, isPreview: boolean) {
    ...
    const unsubscribe = footer.unsubscribe
      ? `<span 
    style="${resetStyles}font-family: ${
          footer.font ? this.getFontFamily(footer.font) : this.defaultFont
        };font-size:${footer.fontSize || "14px"};color:${
          footer.unsubscribeColor || ConstantsData.defaultUnsubscribeColor
        }">
    <a style="${resetStyles}font-family: ${
          footer.font ? this.getFontFamily(footer.font) : this.defaultFont
        };font-size:${footer.fontSize || "14px"};color:${
          footer.unsubscribeColor || ConstantsData.defaultUnsubscribeColor
        }"
    ...
    <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%"
                ...
                <tr>
                    <td
                        ...
                        <tr> 
                        <td style="${resetStyles}word-wrap:break-word;max-width:630px;margin:20px;font-family: ${
      footer.font ? this.getFontFamily(footer.font) : this.defaultFont
    };font-size:${footer.fontSize ? footer.fontSize : "14px"}"> 
                        <![endif]-->
                        ...
                        <div 
                        style="${resetStyles}margin:20px 10px;font-family: ${
      footer.font ? this.getFontFamily(footer.font) : this.defaultFont
    };font-size:${footer.fontSize ? footer.fontSize : "14px"}">
                        ${unsubscribe}
                        </div>
  ...
  getBlock(block: BlockBean, cols: number = 1) {
    if (
      ((block.type === BlockType.Image || block.type === BlockType.Video) &&
        !block.src) ||
  ...
  getVideo(block: BlockBean, cols: number) {
    let imageText = "";
    if (block.src) {
      const tableBodyElem = ` <tr>
            <td></td>
            <td rowspan="2"> 
              <img src="${
                block.src
              }" width="100%" style="max-width: 100%; height: auto; display: block;" alt="${
        block.altTxt || "image"
      }" onerror="this.src='';">
            </td>
          </tr>
  ...
  getButton(block: BlockBean) {
    ...
    if (block.link) {
      ...
      <a ${hrefText}
        ...
        color: ${block.color}; 
        font-family: ${
          block.font ? this.getFontFamily(block.font) : this.defaultFont
        }; 
